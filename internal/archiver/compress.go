...
func Compress(input io.Reader, output io.Writer) error {
	...
	if _, err := output.Write([]byte(fileSignature)); err != nil {
		return err
	}

	buf8 := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf8, uint64(len(data)))
	if _, err := output.Write(buf8); err != nil {
		return err
	}
	...
	if _, err := output.Write(tableBuf); err != nil {
		return err
	}

	writer := bitio.NewBitWriter(output)
	for _, b := range data {
		ci := codeMap[b]
